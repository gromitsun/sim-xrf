...
ray=np.load("scatt_bg3_wpw.npz")['ray']
comp=np.load("scatt_bg3_wpw.npz")['comp']
xrf=np.load("scatt_bg3_wpw.npz")['xrf']
subtend=np.load("scatt_bg3_wpw.npz")['subtend']

matrix_density = 1
element_density = 7.140
...
c = np.logspace(-9,-3,20).reshape(-1,1)
im = 9+18*.02*u_scatt/(c*u_xrf)
plt.figure()
