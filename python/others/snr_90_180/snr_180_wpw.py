...
plt.figure('snr_sa_180_wpw')
ax1 = plt.gca()
ax2 = ax1.twinx()

openings = np.load('snr_wpw180_100nm.npz')['openings']
snr = np.load('snr_wpw180_100nm.npz')['snr']*np.sqrt(n)
...
snr = np.load('snr_wpw180_1um.npz')['snr']*np.sqrt(n)
p2b = np.load('snr_wpw180_1um.npz')['p2b']

ax1.plot(openings,snr, 'r--', label=r'1 um')
ax2.plot(openings,p2b*10, 'b--', label=r'1 um ($\times$ 10)')

snr = np.load('snr_wpw180_10um.npz')['snr']*np.sqrt(n)
p2b = np.load('snr_wpw180_10um.npz')['p2b']

ax1.plot(openings,snr, 'r-.', label=r'10 um')
ax2.plot(openings,p2b*100, 'b-.', label=r'10 um ($\times$ 100)')


snr = np.load('snr_wpw180_100um.npz')['snr']*np.sqrt(n)
p2b = np.load('snr_wpw180_100um.npz')['p2b']
