...
s_protein = sample(p1,1.05,20e-7)

s_water = sample(water,1,1e-4)

s = sample_multilayer([s_water, s_protein, s_water])
...
total_arr = []
signal_arr = []
t_arr = np.linspace(10e-7,10e-4,100)
for t in t_arr:
	s_water = sample(water,1,t)
	s = sample_multilayer([s_water, s_protein, deepcopy(s_water)])
	# calculate spectrum
...
np.savez('wpw180_thin.npz',snr = SNR, p2b = p2b, t_arr = t_arr)
# show all plots
plt.show()
